...
if(true)
{
	...
	$smarty->assign('pre_mobile_select', $pre_mobile_select);
...
function action_send_mobile_code ()
{
	...
	{
		exit("Incorrect format of mobile phone number");
		return;
	}
	...
	// 短信内容
    $content = array($_CFG['sms_register_tpl'], "{\"number\":\"$mobile_code\"}",$_CFG['sms_sign']);
...
// 		}
		...
		else if($register_type == "mobile")
		{
            ...
            //jack add 
            $pre_mobile_phone = ! empty($_POST['pre_mobile_phone']) ? trim($_POST['pre_mobile_phone']) : '';
            //end
