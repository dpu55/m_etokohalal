...
// 	}
	...
	**/
	...
	show_message($_LANG['m_f_1056'], array($_LANG['m_f_1057'], $_LANG['back_up_page']), array('user.php?act=login', 'index.php'), 'info');
...
{
	exit($_LANG['m_f_1058'] . $function_name . $_LANG['m_f_1059']);
}
...
function action_act_login ()
{
	...
	if(is_telephone($username))
	{
		...
		if($kkk > 1)
		{
			$smarty->assign('lang', $_LANG);
			$smarty->assign('action', 'login');
			$smarty->assign('error', $_LANG['m_f_1060']);
			$smarty->display('chat_passport.dwt');
			return;
		}
		if($username_e)
...
function action_chat ()
{
	...
	if(! empty($_REQUEST['chat_goods_id']))
	{
		...
		$tab_items[] = array(
			"id" => "chat_goods","name" => $GLOBALS['_LANG']['m_f_1061']
		);
	...
	if(! empty($_REQUEST['chat_order_id']))
	{
		...
		$tab_items[] = array(
			"id" => "chat_order","name" => $GLOBALS['_LANG']['m_f_1062']
		);
	...
	if(! empty($_REQUEST['chat_supp_id']) && $_REQUEST['chat_supp_id'] != 0)
	{
		...
		$tab_items[] = array(
				"id" => "chat_supp", "name" => $GLOBALS['_LANG']['m_f_1063']
		);
	...
	if(true)
	{
		...
		$tab_items[] = array(
			"id" => "chat_order_list","name" => $GLOBALS['_LANG']['m_f_1064']
		);
	...
	if(! empty($customers))
	{
		...
		if($poliy == 0)
		{
			foreach($customers as $customer)
			{
				...
				if($status == $GLOBALS['_LANG']['m_f_1065'] || $status == $GLOBALS['_LANG']['m_f_1066']) //
				{
...
// 					}
		...
		if(empty($to))
		{
			if($supp_id == -1){
				// 所有客服忙碌状态，提示web端
				$smarty->assign('system_notice', $GLOBALS['_LANG']['m_f_1067']);
			}else{
				// 所有客服忙碌状态，提示web端
				$smarty->assign('system_notice', $GLOBALS['_LANG']['m_f_1068']);
			}
		}
		else
		{
			...
			// 所有客服忙碌状态，提示web端
			$smarty->assign('system_notice', $GLOBALS['_LANG']['m_f_1070'].'<span class="kf_name">' . $to['cus_name'] . '</span>'.$GLOBALS['_LANG']['m_f_1069']);
		}
	}
	else
	{
...
function action_authfail ()
{
	...
	{
		$result = array(
			'error' => 1,'message' => $GLOBALS['_LANG']['m_f_1071'].'&nbsp;<a href="chat.php?act=chat"><strong>'.$GLOBALS['_LANG']['m_f_1072'].'</strong></a>&nbsp;, '.$GLOBALS['_LANG']['m_f_1073'].'...','content' => ''
		);
	}
	else
	{
