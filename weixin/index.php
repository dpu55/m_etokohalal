...
if ($event['event'] == "subscribe") { //用户关注
    if (intval($followInfo['ecuid']) == 0) {
        ...
        if ($rs === false) {
            echo $weixin->text("系统繁忙,请稍后再试")->reply();
            exit;
        }
        $weixinconfig['followmsg'] .= "关注成功！\r\n系统分配帐号：$username\r\n密码：$pwd";
    ...
    switch ($content) {
        ...
        case "jcbd":
            ...
            if ($followInfo['ecuid'] == 0) {
                echo $weixin->text("您还没有绑定账号!")->reply();
                exit;
            }
            $api->unBindUser($wxid);
            echo $weixin->text("帐号绑定解除！")->reply();
            exit;
            break;
        case "bdhy":
            if ($api->isBindUser($wxid)) {
                echo $weixin->text("您已经绑定帐号，无需重复绑定！")->reply();
                exit;
            }
            echo $weixin->text($weixinconfig['reg_notice'])->reply();
            exit;
            break;
        case "info":
...
//                $text .= "邮箱：{$reMsg['email']}\r\n";
    ...
    if (strpos($content, "article_") !== false) {
        ...
        if ($diy_type == 1) {
            echo $weixin->text($artInfo['description'])->reply();
            exit;
        }
...
if ($event['event'] == "SCAN") {
    ...
    if ($res) {
        if ($res['type'] == 1) {
            ...
            echo $weixin->text($res['content'])->reply();
            exit;
        } elseif ($res['type'] == 4) {
    ...
    if ($api->scanLogin($content, $wxid) === true) {
        echo $weixin->text("您使用扫一扫功能登陆网站成功！")->reply();
        exit;
    }
}
//处理用户的输入
