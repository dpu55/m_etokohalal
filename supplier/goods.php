...
if($act == 'list')
{
    ...
    _wap_assign_header_info($_LANG['y_308']);
    _wap_assign_footer_order_info();
    _wap_display_page('goods_list.htm');
}
...
elseif($act == 'edit')
{
    ...
    echo "Formnya belum... <br/><pre>"; var_dump($goods);
    // $smarty->assign('goods',$goods);
    // _wap_assign_header_info($_LANG['y_311']);
    // _wap_assign_footer_order_info();
    // _wap_display_page('goods_list.htm');
}
...
{
    global $ecs,$db;
    $sql = "SELECT * FROM " . $ecs->table('goods') . " WHERE supplier_id='".$_SESSION['supplier_id']."' AND goods_id = '" .$goods_id. "'";
    $goods = $db->getRow($sql);

    $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '" .$goods_id. "'";
    $images = $db->getAll($sql);

    $data = array('goods' => $goods, 'good_images' => $images);
    return $data;
}
...
function get_all_goods()
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT goods_id, goods_sn, goods_name, goods_weight, concat("Rp", format(shop_price, 0)) as shop_price, concat("Rp", format(market_price, 0)) as market_price, goods_number as number_stock FROM '.$ecs->table('goods').$where.$limit;
