...
// 			die($_LANG['require_login']);
			...
			show_message($_LANG['require_login'], array('</br>'.$_LANG['m_28'], '</br>'.$_LANG['back_home']), array('user.php?act=login', $ecs->url()), 'error', false);
...

{
	...
	show_message($mes, $_LANG['back_up_page'], 'user.php?act=supplier_reg', 'info');
...

{
...

	{
		...
		show_message($_LANG['m_527'], $_LANG['back_up_page'], 'user.php?act=supplier_reg', 'wrong');
...

	{
		...
		show_message($_LANG['m_528'], $_LANG['back_home'], '', 'wrong');
	...
	show_message($_LANG['m_529'],  $_LANG['back_up_page'], 'user.php', 'info');
...

{
	...
	$mobile_phone = trim($_REQUEST['mobile_phone']);
     ...
     //jack add
    $pre_mobile = trim($_REQUEST['pre_mobile']);
    //end
	...
	$result = sendSMS($mobile_phone, $content,$pre_mobile);
...

{
...

	{
		...
		show_message($_LANG['m_530'], $_LANG['home'], $ecs->url(), 'error');
...

{
...

	{
		...
		show_message($_LANG['m_531'], $_LANG['home'], $ecs->url(), 'error', false);

	}

	if(! $info['user_id'])

		show_message($_LANG['m_532'], $_LANG['home'], $ecs->url(), 'error', false);
...

{
	...
	require_once "googleConfig.php"; // Google login
	require_once "FacebookConfig.php"; // Facebook login
	...
	$smarty->assign('facebookURL', $facebookURL);
...

{
...

	{
...

		{
			...
			show_message($_LANG['m_511'], $_LANG['relogin_lnk'], 'user.php', 'error');
...

{
...

	{
		...
		show_message($_LANG['m_544'], $_LANG['back_up_page'], 'user.php?act=profile');
...

	{
		...
		show_message($_LANG['m_545'], $_LANG['back_up_page'], 'user.php?act=profile');
...

{
...

	{
		...
		show_message($_LANG['cmt_lang']['captcha_not_null'], $_LANG['m_166'], 'user.php?act=update_email', 'error');
...

	{
		...
		show_message($_LANG['invalid_captcha'], $_LANG['m_166'], 'user.php?act=update_email', 'error');
...

	{
...

		{
			...
			show_message($_LANG['sendemail_false']);
...

{
...

	{
		...
		show_message($_LANG['cmt_lang']['captcha_not_null'], $_LANG['m_166'], 'user.php?act=re_binding_email', 'error');
...

	{
		...
		show_message($_LANG['invalid_captcha'], $_LANG['m_166'], 'user.php?act=re_binding_email', 'error');
...

{
...

	{
...

		{
			...
			show_message($_LANG['m_550'],$_LANG['m_551'], 'user.php?act=account_security');
...

{
...

	{
		...
		if($v_kuaidi['shipping_name_2'] == $_LANG['m_25'])

		{
...

				{
...

					{
						...
						case 1:
							...
							$shipping_info .= $_LANG['m_556']." (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";

							break;

						case 2:

							$shipping_info .= $_LANG['m_557']." (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";
						...
						case 3:
							...
							$shipping_info .= $_LANG['m_558']."" . $postman_info['postman_name'] . ", ".$_LANG['m_54'] . $postman_info['mobile'] . " (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";

							break;

						case 4:

							$shipping_info .= $_LANG['m_559']." (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";

							break;

						case 5:

							$shipping_info .= $_LANG['m_560']." (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";

							break;

						case 6:

							$shipping_info .= $_LANG['m_561']." (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";

							break;

						case 7:

							$shipping_info .= $_LANG['m_562']." (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";
...

			{
				...
				$orders[$k_kuaidi]['result_content'] = $_LANG['m_563'];
...

{
	...
	$sql = "SELECT og.*, o.add_time, o.shipping_time_end, o.order_id, g.goods_thumb,g.shop_price, s.shaidan_id, s.pay_points AS shaidan_points, s.status AS shaidan_status,
			...
			c.status AS comment_status,g.supplier_id,ifnull(ssc.value,'".$_LANG['m_409']."') AS shopname
...

{
	...
	$rec_id = $_POST['rec_id'];
        ...
        if($res_comment_id){
            ...
            echo "<script>alert('".$_LANG['m_564']."');location='user.php?act=my_comment';</script>";
...

{
	...
	$hide_username = intval($_POST['hide_username']);
        ...
        if($res_shaidan_id){
            ...
            echo "<script>alert('".$_LANG['m_567']."');location='user.php?act=my_comment';</script>";
...

	{
...

		{
			...
			$db->query("INSERT INTO " . $ecs->table('account_log') . "(user_id, rank_points, pay_points, change_time, change_desc, change_type) " . "VALUES ('$info[user_id]', 0, '" . $pay_points . "', " . gmtime() . ", '".$_LANG['m_610']."', '99')");
...
function action_set_address(){
	...
	$ecs = $GLOBALS['ecs'];
        ...
        if($db->query("UPDATE " . $ecs->table('users') . " set address_id = $address_id  WHERE user_id='$user_id'")){
            ...
            show_message($_LANG['m_570'], $_LANG['address_list_lnk'], 'user.php?act=address_list');
...

{
...

	{
		...
		show_message($_LANG['m_571'], $_LANG['order_list_lnk'], 'user.php?act=order_list', 'info');
...

{
...

	{
		...
		show_message($_LANG['m_572'], $_LANG['m_573'], 'user.php?act=vc_login');
...

	{
		...
		show_message($_LANG['m_574'], $_LANG['m_575'], 'user.php?act=vc_login');
...

	{
		...
		show_message($_LANG['m_576'], $_LANG['m_575'], 'user.php?act=vc_login');
...

	{
		...
		show_message($_LANG['m_577'], $_LANG['m_578'], 'user.php?act=vc_login');
...

	{
		...
		show_message($_LANG['m_579'], $_LANG['m_580'], 'user.php?act=vc_login');
...

	{
		...
		show_message($_LANG['m_581'],$_LANG['m_580'], 'user.php?act=vc_login');

	}



	$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('user_account') . ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid)' . " VALUES ('$user_id', '', '$vcrow[type_money]', '" . gmtime() . "', '" . gmtime() . "', '', '".$_LANG['m_611']."', '0', ".$_LANG['m_612']."'$vc_sn', 1)";

	$GLOBALS['db']->query($sql);

	log_account_change($user_id, $vcrow['type_money'], 0, 0, 0, $_LANG['m_582'] . $vc_sn, ACT_OTHER);
	...
	show_message($_LANG['m_583'], $_LANG['back_up_page'], 'user.php?act=vc_login');
...

{
...

	{
		...
		show_message($_LANG['m_572'], $_LANG['m_573'], 'user.php?act=tg_login');
...

	{
		...
		show_message($_LANG['m_584'],$_LANG['m_575'], 'user.php?act=tg_login');
...

	{
		...
		show_message($_LANG['m_576'],$_LANG['m_575'], 'user.php?act=tg_login');
...

	{
		...
		show_message($_LANG['m_585'] . local_date('Y-m-d H:i:s', $tgrow['use_start_date']), $_LANG['m_586'], 'user.php?act=tg_login');
...

	{
		...
		show_message($_LANG['m_587'], $_LANG['m_588'], 'user.php?act=tg_login');
...

	{
		...
		show_message($_LANG['m_589'], $_LANG['m_588'], 'user.php?act=tg_login');
...

{
	...
	show_message($_LANG['m_590'], $_LANG['m_591'], 'user.php?act=tg_order');
...

	{
...

		{
...

				{
...

					{
...

						{
							...
							show_message($_LANG['m_592'],$_LANG['m_166'], 'user.php?act=account_security', 'success');
...

						{
							...
							show_message($_LANG['m_593'], $_LANG['m_166'], 'user.php?act=account_security', 'fail');
...

			{
				...
				show_message($_LANG['m_154'], $_LANG['m_166'], 'user.php?act=update_surplus_password', 'error');
...

{
...

	{
		...
		show_message($_LANG['m_599'], $_LANG['m_166'], 'user.php?act=forget_surplus_password', 'error');
...

	{
...

		{
...

			{
				...
				show_message($_LANG['m_600'], $_LANG['m_166'], 'user.php?act=forget_surplus_password', 'error');
...

			{
				...
				show_message($_LANG['m_601'], $_LANG['m_166'], 'user.php?act=forget_surplus_password', 'error');
...

		{
...

			{
				...
				show_message($_LANG['m_602'], $_LANG['m_166'], 'user.php?act=forget_surplus_password', 'error');
...

			{
				...
				show_message($_LANG['m_603'], $_LANG['m_166'], 'user.php?act=forget_surplus_password', 'error');
...

			{
...

				{
					...
					show_message($_LANG['m_604'],$_LANG['m_166'], 'user.php?act=forget_surplus_password', 'error');
...

				{
...

					{
						...
						show_message($_LANG['m_605'], $_LANG['m_166'], 'user.php?act=account_security', 'success');
...

{
...

	{
		...
		$content = $_LANG['m_607'] . $verifycode;
		...
		if($ret == $_LANG['m_608'] && $db->affected_rows() == 1)

		{

			$result['result'] = 'success';

			$result['message'] = $_LANG['m_608'];
...

{
	...
	$order_status = array(
		...
		'0' => $_LANG['m_518'],'1' =>$_LANG['m_519'],'2' => $_LANG['m_296']
...
function get_goods_info_wap($goods_id){
    ...
    if($goods['supplier_id'] == '0'){
        ...
        $goods['supplier_name']=" ".$_LANG['m_409'];
