...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_template();
    $smarty->assign('keywords',    $_LANG['m_f_1150']);
    $smarty->assign('description',   $_LANG['m_f_1150']); 
    $smarty->assign('page_title',      $_LANG['m_f_1150']);    // 页面标题
    $smarty->assign('ur_here',          '<a href=".">'.$_LANG['home'].'</a> &gt; '.$_LANG['m_f_777']);  // 当前位置
...
elseif ($_REQUEST['act'] == 'take')
{
	...
	{
		show_message($_LANG['m_f_1079'], $_LANG['back_up_page'], 'takegoods.php', 'error');
	}
	else
	{ 
		...
		{
			show_message($_LANG['m_f_1151'], $_LANG['back_up_page'], 'takegoods.php', 'error');
		}
	}
    $add_time = gmtime();
	...
	if ($_CFG['takegoods_send_email'] =='1' || $_CFG['takegoods_send_sms'] == '1' )
	{
		...
		$content1 = $_LANG['member_name'].' '. $take_user_name .$_LANG['m_f_1152'].local_date('Y-m-d H:i:s')." ".$_LANG['m_f_1153'];
		$content2 = $_LANG['m_f_1154'].' '.local_date('Y-m-d H:i:s')." ".$_LANG['m_f_1155'];	
		if ($_CFG['takegoods_send_email'] == '1')
		{	
			$admin_email = $db->getOne("select email from ". $ecs->table('admin_user') ." where user_id=1 ");
			send_mail($_CFG['shop_name'], $admin_email, $_LANG['m_f_780'].' '. $takegoods_sn_68ecshop .' '.$_LANG['m_f_1156'], $content1, 0);
			send_mail($_CFG['shop_name'], $email, $_LANG['m_f_780'].' '. $takegoods_sn_68ecshop .' '.$_LANG['m_f_1156'], $content2, 0);
		}
	...
	show_message($_LANG['m_f_1157']. $country_name .' '.$province_name.' '. $city_name. ' '.$district_name.' '.$address.' '.$_LANG['m_f_1158'], ''.$_LANG['m_f_1159'], '.');
