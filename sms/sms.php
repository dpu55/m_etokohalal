...
function sendSMS($mobile_phone, $content){
    ...
    include "hy_config.php";
    $x_userid = $hy_appkey;
    $x_password = $hy_secretkey;
    $msg = $content['0'];
    $bianliang = json_decode($content['1'],true);
    $s_bl = array();
    if(!empty($bianliang)){
        foreach($bianliang as $k => $v){
            $s_bl[] = $v;
        }
    }
    $s_bl_s = implode(",",$s_bl);
    $qianming = $content['2'];
    $msg = sprintf($msg,$s_bl_s,$qianming);
    $smsapi = "https://smsgw.sprintasia.net/api/msg.php?u=".$x_userid."&p=".$x_password."&d=".$mobile_phone."&m=".urlencode($msg); //短信网关
    $apiresult=file_get_contents($smsapi);
    $exp = explode("-",$apiresult);
    if($exp['0']=='0'){
        return true;
    }
    else{
        if ($hy_showbug == true) {
            ...
            switch($exp['0']){
                ...
                case "2":
                    ...
                    break;
                case "3":
                    $error_msg = 'Invalid MSISDN / Destination';
                    break;
                case "4":
                    $error_msg = 'Invalid Username / Password';
                    break;
                case "5":
                    $error_msg = 'Membership is not available';
                    break;
                case "6":
                    $error_msg = 'IP is not whitelist';
                    break;
                case "7":
                    $error_msg = 'Not enough credit';
                    break;
                case "8":
