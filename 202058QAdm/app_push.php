...
elseif($_REQUEST['act'] == 'save_setting')
{
    ...
    if(count($app_names) == count($app_keys) && count($app_keys) == count($app_secrets))
    {
		...
		if($db->query($sql))
		{
			sys_msg($_LANG['x_174'],0,array(array('text'=>$_LANG['x_175'],'href'=>'app_push.php?act=setting')));
		}
		else
		{
			sys_msg($_LANG['modify_failure'],1);
		}
    }
    else
    {
        sys_msg($_LANG['x_176'],1);
    }
}
...
elseif($_REQUEST['act'] == 'do_push_message')
{
	...
	if(empty($ids))
    {
        sys_msg($_LANG['x_177'] ,1);
    }
	if(empty($content))
	{
		sys_msg($_LANG['x_178'],1);
	}
	$exception = '';
	$push_setting = get_push_setting();
	$apps = array();
    ...
    {
        sys_msg($exception,1,array(array('text'=>$_LANG['x_179'],'href'=>'app_push.php?act=push_message')));
    }
    else
    {
        sys_msg($_LANG['x_180'],0,array(array('text'=>$_LANG['x_179'],'href'=>'app_push.php?act=push_message')));
    }
}
...
function do_app_push($content,$extras,$apps)
{
    ...
    $notification = array();
	$title = $GLOBALS['_LANG']['x_181'];
	foreach($extras as $key => $val)
	{
		if($key == 'goods')
		{
			$title = $GLOBALS['_LANG']['x_182'];
		}
		else if($key == 'article')
		{
			$title = $GLOBALS['_LANG']['x_183'];
		}
		else if($key == 'url')
		{
			$title = $GLOBALS['_LANG']['x_184'];
		}
	}
    $ios = array();
	$ios['title'] = $title;
