...
switch ($act){
		...
		if($_POST){
			...
			$ret = $db->query ( 
					...
					`auto_reply`='$auto_reply'
					 WHERE `id`=1;" );
			$link [] = array ('href' => 'weixin.php?act=config','text' => $_LANG['x_10']);
			if ($ret) {
				sys_msg ( $_LANG['x_237'] , 0, $link );
			} else {
				sys_msg ($_LANG['x_240'], 0, $link );
			}
		}else{
			$ymd = date('Y-m-d');
	...
	case "delmenu":
		...
		$link [] = array ('href' => 'weixin.php?act=menu','text' => $_LANG['x_241']);
		update_menu();
		sys_msg ( $_LANG['x_242'], 0, $link );
		break;
	case "addmenu"://添加菜单
	...
	case "keywords":
		...
		if($id){
			if($_POST){
				...
				$ret = $GLOBALS['db']->query("
					...
					`jf_maxnum`='$jf_maxnum'
					 WHERE `id`=$id;
				");
				$link [] = array ('href' => 'weixin.php?act=keywords','text' => $_LANG['x_17']);
				if ($ret) {
					sys_msg ( $_LANG['x_246'], 0, $link );
				} else {
					sys_msg ($_LANG['x_247'], 0, $link );
				}
			}
			...
			$smarty->assign ( 'keywords', $keywords );
			if($diy_type > 0){
				$smarty->assign('action_link',  array('text' => $_LANG['x_248'], 'href'=>'weixin.php?act=addkey'));
	...
	case "fans":
		...
		if($fake_id != '' ){
			...
			sys_msg ($_LANG['x_190'], 0, $link );
		}
		$user_list = user_list();
		$smarty->assign('ur_here',      $_LANG['x_249']);
	...
	case "notice":
		if($_POST){
			...
			$ret = $db->query (
					...
					WHERE `id`=1;" );
					$link [] = array ('href' => 'weixin.php?act=notice','text' => $_LANG['x_20']);
			if ($ret) {
					sys_msg ( $_LANG['x_237'], 0, $link );
			} else {
				sys_msg ($_LANG['x_240'], 0, $link );
			...
			$smarty->assign('ur_here',      $_LANG['x_20']);
			$ret = $db->getRow ( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_config') . " WHERE `id` = 1" );
	...
	case "reg":
		if($_POST){
			...
			$ret = $db->query (
					...
					`reg_type`='$reg_type'
					WHERE `id`=1;" );
			$link [] = array ('href' => 'weixin.php?act=reg','text' => $_LANG['x_250']);
			if ($ret) {
				sys_msg ( $_LANG['x_237'], 0, $link );
			} else {
				sys_msg ($_LANG['x_240'], 0, $link );
			...
			$smarty->assign('ur_here',      $_LANG['x_250']);
			$ret = $db->getRow ( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_config') . " WHERE `id` = 1" );
	...
	case "fansmsg":
		...
		if($_POST['content'] && $fake_id){
			...
			$link [] = array ('href' => 'weixin.php?act=fansmsg&fake_id='.$fake_id,'text' => $_LANG['x_251']);
			if ($ret) {
				sys_msg ( $_LANG['attradd_succed'], 0, $link );
			} else {
				sys_msg ( $_LANG['x_252'], 0, $link );
			}
		}else{
			$smarty->assign('action_link',  array('text' => $_LANG['x_249'], 'href'=>'weixin.php?act=fans'));
			$fake_id = getstr($_GET['fake_id']);
			if(!$fake_id){
				sys_msg ($_LANG['x_253'], 0, $link );
			}
			...
			$smarty->assign('ur_here', $_LANG['x_251']);
			$smarty->assign('fake_id', $fake_id);
			$smarty->display ( 'weixin/wx_fansmsg.html' );
		}
		break;
	case "qcode":
		$smarty->assign('action_link',  array('text' => $_LANG['x_14'], 'href'=>'weixin.php?act=addqcode'));
		$qcode_list = qcode_list();
		$smarty->assign('ur_here',      $_LANG['x_254']);
	...
	case "addqcode":
		$id = intval($_REQUEST['id']);
		if($_GET['do'] == 'del' && $id>0){
			$link [] = array ('href' => 'weixin.php?act=qcode','text' =>$_LANG['x_255'] );
			$ret = $db->query("delete from " . $GLOBALS['ecs']->table('weixin_qcode') . " where id=$id");
			if ($ret) {
				sys_msg ( $_LANG['x_242'], 0, $link );
			} else {
				sys_msg ( $_LANG['x_256'], 0, $link );
			}
		}
		if($_POST){
			$type = intval($_POST['type']);
			...
			if($content){
				if(!$id){
					$link [] = array ('href' => 'weixin.php?act=addqcode','text' => $_LANG['x_14'] );
			...
			$smarty->assign('action_link',  array('text' => $_LANG['x_254'], 'href'=>'weixin.php?act=qcode'));
			$ret = $db->getRow("select * from " . $GLOBALS['ecs']->table('weixin_qcode') . " where id=$id");
			$smarty->assign('ur_here', $_LANG['x_14']);
	...
	case "news":
		...
		if($_POST){
			...
			$link [] = array ('href' => 'weixin.php?act=newslist','text' =>$_LANG['x_259']);
			sys_msg ( "auto_do", 0, $link, false );
		}else{
			$smarty->assign('action_link',  array('text' =>$_LANG['x_259'], 'href'=>'weixin.php?act=newslist'));
			$smarty->assign('action_link2',  array('text' => $_LANG['article_add'], 'href'=>'article.php?act=add'));
			$smarty->assign('ur_here',$_LANG['x_19']);
		...
		break;
	case "newslist":
		$smarty->assign('action_link',  array('text' => $_LANG['x_19'], 'href'=>'weixin.php?act=news'));
		...
		$smarty->assign('ur_here', $_LANG['x_259']);
		$smarty->assign('corn', $ret);
	...
	case "view":
		...
		$ret = $db->getRow("select * from " . $GLOBALS['ecs']->table('weixin_corn') . " where id={$id}");
		if($ret){
			$smarty->assign('ur_here', $_LANG['x_260']);
			$content = unserialize($ret['content']);
	...
	case "addconfig":
		...
		if($id > 1 && $_GET['up'] == 1){
			update_menu($id);
			$link [] = array ('href' => 'weixin.php?act=addconfig','text' => $_LANG['x_262']);
			sys_msg ( $_LANG['attradd_succed'], 0, $link );
		} 
		if($id > 1 && $_GET['up'] == 2)
		{
...
(`weburl`,`click`,`id`) VALUE ('$weburl',0,$id)");
				...
				$link [] = array ('href' => 'weixin.php?act=oauth','text' => $_LANG['x_263']);
				sys_msg ( $_LANG['attradd_succed'], 0, $link );
 ...
 where oid={$oid}");
			...
			$smarty->assign('action_link',  array('text' => $_LANG['x_264'], 'href'=>'weixin.php?act=oauth&t=add'));
			$baseurl = $_SERVER['SERVER_NAME'] ? "http://".$_SERVER['SERVER_NAME']."/" : "http://".$_SERVER['HTTP_HOST']."/";
			$smarty->assign('baseurl',$baseurl);
	...
	case "qiandao":
		if($_POST){
			...
			$ret = $db->query(
	    		...
	    		`addnum`='$addnum'
	    		WHERE `cid`=1;");
			$link [] = array ('href' => 'weixin.php?act=qiandao','text' => $_LANG['x_265']);
            if ($ret) {
                sys_msg ($_LANG['attradd_succed'], 0, $link );
            } else {
                sys_msg ( $_LANG['x_252'], 0, $link );
...
//        }
	...
	case "delkey":
		$id = intval($_REQUEST['id']);
		$link [] = array ('href' => 'weixin.php?act=keywords&t=1','text' => $_LANG['x_266']);
		$count = $GLOBALS['db']->query("DELETE FROM " .$GLOBALS['ecs']->table('weixin_keywords')." WHERE id = '$id'");
		if($count > 0)
		{
			sys_msg($_LANG['x_267'],0,$link);
		}
		else
		{
			sys_msg($_LANG['x_268'],0,$link);
		}
		break;
	case "addkey":
		$id = intval($_REQUEST['id']);
		if($_POST){
			...
			$link [] = array ('href' => 'weixin.php?act=addkey','text' => $_LANG['x_142']);
			$link [] = array ('href' => 'weixin.php?act=keywords&t=1','text' => $_LANG['x_266']);
			sys_msg ($_LANG['attradd_succed'], 0, $link );
		}else{
			$smarty->assign('action_link',  array('text' => $_LANG['x_266'], 'href'=>'weixin.php?act=keywords&t=1'));
			$smarty->assign('ur_here',$_LANG['x_271']);
			if($id > 0){
...
function update_menu($id=1){
	...
	if($res === false){
		sys_msg ( $GLOBALS['_LANG']['x_272'].$weixin->errMsg, 1, $link );
	}else{
		return true;
	}
}
...
function pushToUserMsg($fake_id,$type="text",$msg=array(),$sendtime=0){
	...
	if($user && $user['fake_id'] && $user['isfollow'] == 1){
		...
		$GLOBALS['err']->add($GLOBALS['_LANG']['x_273']);
		return false;
	}
}
...
function get_corn($fake_id)
{
	 ...
	 {
		  $arr[$key]['nickname'] = $GLOBALS['_LANG']['x_274'];
