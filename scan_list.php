...
if($act == 'view'){
	...
	$smarty->assign('page_title', $_LANG['m_f_1115']);    // 页面标题
...
elseif($act == 'addcart'){
	...
	$json  = new JSON;
	$info = array(1=>$_LANG['m_f_1117'],2=>$_LANG['m_f_1118'],3=>$_LANG['m_f_1119'],4=>$_LANG['m_f_1120'],5=>$_LANG['m_f_1121'],6=>$_LANG['m_f_1122']);
	if(isset($_SESSION['saodata']) && !empty($_SESSION['saodata'])){
		...
		$result = array('error'=>count($fail),'content'=>$_LANG['m_f_1123'].':','fail'=>$fail,'ok'=>$ok);
		die($json->encode($result));

	}else{
		$result = array('error'=>-1,'content'=>$_LANG['m_f_1124']);
		die($json->encode($result));
	}
}
