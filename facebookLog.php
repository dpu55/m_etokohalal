<?php
    ...
    $username = explode("@", $_GET['email']);
    $data['id'] = $_GET['id'];
    $data['email'] = $_GET['email'];
    $data['gender'] = "";
    $data['picture'] = $_GET['pic'];
    ...
    $sql="select * from ". $ecs->table('users') ." where email='".$data['email']."'";
	...
	if($user_check) {
		...
		$message = "Hi, ada 1 member join dengan nama: ".$data['username']." dan email: ".$data['email']." via Google";
		sendTele($message) ;
    }
    ...
    function action_register ($data)
	{
		...
		$other['picture'] = isset($data['picture']) ? $data['picture'] : '';
    ...
    function action_login ($data)
	{
		...
		$sql = "UPDATE " . $ecs->table('users') . "SET password= '" . $new_password . "',ec_salt='" . $ec_salt . "', picture='" .$data['picture']."'" . " WHERE email='".$data['email']."'";
	    $db->query($sql);
