		...
		shopSearch:function(args){
		    		...
		    		$(n).bind({
			    		...
			    		blur: function() {
			    			if(priceElem.length == 2 ) {
				    			...
				    			if(minPrice == "") {
				    				priceElem.eq(0).val("Rp");
				    			}
				    			if(maxPrice == "") {
				    				priceElem.eq(1).val("Rp");
				    			}
				    		}
			    		},
			    		change : function() {
		    ...
		    if( param['isShowPriceSift'] == "true" ){
		    	if(minPrice != null && maxPrice != null) {
		    		minPrice= minPrice.replace("Rp","");
		    		if(minPrice=="" || minPrice=="Rp" ) {
		    			minPrice=0;
		    		}
		    		maxPrice= maxPrice.replace("Rp","");			
		    		if(maxPrice==""|| maxPrice=="Rp") {
		    			maxPrice=0;
		    		}	
		    	}
			}else{
...
//			}
	...
	function priceOnblur(mod_id){
		...
		if( minPrice=="" ){
			$(mixPrice_input).val("Rp");
		}
		if( maxPrice=="" ){
			$(maxPrice_input).val("Rp");
		}
		...
		if($.isArray(html)){
			for(var i=0;i<html.length && i<10;i++){
				...
				if(dataElement.cid!=null){
					...
					'" keyword="'+dataElement.keyword+'">cari di kategori <em>'+dataElement.cname+'</em></a>';
				}
