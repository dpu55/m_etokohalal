...
function action_send_mobile_code ()
{
    ...
    $pre_mobile_phone = trim($_POST['pre_mobile_phone']);
	...
	/* 发送激活验证短信 */
    $result = sendSMS($mobile_phone, $content,$pre_mobile_phone);
	if($result)
	{
...
function action_validate ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1077'], 'url' => 'security.php'
		)));
	}
	...
	else if($validate_type == 'password')
	{
		...
		{
			exit(json_encode(array(
				'error' => 1, 'content' => $_LANG['m_f_1133'], 'url' => ''
			)));
		}
	}
	else
	{
		/* 无效的注册类型 */
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1078'], 'url' => ''
		)));
	}
...
function action_to_password_reset ()
{
	...
	{
		show_message($_LANG['m_f_1079'], $_LANG['back_up_page'], 'security.php?act=password_reset', 'info');
	}
...
function action_do_password_reset ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1079'], 'url' => 'security.php'
		)));
	}
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_194'], 'url' => ''
		)));
	}
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1080'], 'url' => ''
		)));
	}
	else
	{
...
function action_to_email_binding ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message($_LANG['m_f_1079'], $_LANG['back_up_page'], 'security.php?act=payment_password_reset', 'info');
	}
...
function action_do_email_binding ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1079'], 'url' => ''
		)));
	}
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1134'], 'url' => ''
		)));
	}
	else
	{
...
function action_email_validate ()
{
	...
	if(empty($email))
	{
		show_message($_LANG['m_f_1135'], array(
			$_LANG['m_f_1136'], $_LANG['m_f_273']
		), array(
...
function action_do_email_validate ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1137'], 'url' => ''
		)));
	}
	else
	{
...
function action_to_mobile_binding ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message($_LANG['m_f_1138'], array(
			$_LANG['m_f_1139'], $_LANG['m_f_273']
		), array(
...
function action_do_mobile_binding ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1140'], 'url' => ''
		)));
	}
	else
	{
...
function action_mobile_validate ()
{
	...
	if(empty($mobile))
	{
		show_message($_LANG['m_f_1138'], $_LANG['m_f_1139'], 'security.php?act=mobile_binding', 'info');
	}
...
function action_do_mobile_validate ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1141'], 'url' => ''
		)));
	}
	else
	{
...
function action_do_payment_password_reset ()
{
	...
	if(! empty($surplus_password))
	{
		...
		{
			exit(json_encode(array(
				'error' => 1, 'content' => $_LANG['m_f_1142'], 'url' => ''
			)));
		}
	}
	else
	{
		exit(json_encode(array(
			'error' => 1, 'content' =>$_LANG['m_f_1143'], 'url' => ''
		)));
	}
}
...
function action_do_payment_password_close ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message($_LANG['m_f_1079'], $_LANG['m_f_1144'], 'security.php', 'info');
	}
	...
	else
	{
		// 释放变量
		$_SESSION['security_validate'] = false;
		show_message($_LANG['m_f_1145'], $_LANG['m_f_1144'], 'security.php', 'info');
	}
}
...
function action_to_sync_supplier ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message($_LANG['m_f_1145'], $_LANG['back_up_page'], 'security.php?act=sync_supplier', 'info');
	}
...
function action_do_sync_supplier ()
{
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1145'], 'url' => 'security.php'
		)));
	}
	...
	if($result == false)
	{
		$_SESSION['security_validate'] = false;
		exit(json_encode(array(
			'error' => 1, 'content' => $_LANG['m_f_1146'], 'url' => 'security.php'
		)));
	}
	else
	{
...
function get_takegoods_orders ($user_id, $num = 10, $start = 0)
{
    $_LANG = $GLOBALS['_LANG'];
	$order_status = array(
		'0' => $_LANG['m_f_1081'], '1' => $_LANG['m_f_477'], '2' => $_LANG['m_f_19']
	);
	/* 取得订单列表 */
	$arr = array();
...
function get_validate_types ($user_id)
{
	...
	if($user_info == false)
	{
		show_message($_LANG['m_f_266'], $GLOBALS['_LANG']['relogin_lnk'], 'findPwd.php', 'error');
	}
	...
	if(isset($mobile_phone) && ! empty($mobile_phone) && $mobile_validate == 1)
	{
		...
		$validate_types[] = array(
			'type' => 'mobile_phone', 'name' => $_LANG['m_f_264'], 'value' => $mobile_phone
		);
	}
	if(isset($email) && ! empty($email) && $email_validate == 1)
	{
		...
		$validate_types[] = array(
			'type' => 'email', 'name' =>$_LANG['m_f_265'], 'value' => $email
		);
	}
	...
	{
		$validate_types[] = array(
			'type' => 'password', 'name' => $_LANG['m_f_263'], 'value' => $_SESSION['user_name']
		);
	}
