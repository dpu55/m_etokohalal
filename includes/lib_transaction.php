...
function get_user_bouns_list($user_id, $num = 10, $start = 0, $suppid=-1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['supplier_name'] = $row['supplier_id']>0? $GLOBALS['db']->getOne("select value from ". $GLOBALS['ecs']->table('supplier_shop_config')." where code = 'shop_name' and supplier_id = '".$row['supplier_id']."'"):$GLOBALS['_LANG']['m_409'];
        $arr[] = $row;
    }
    return $arr;
