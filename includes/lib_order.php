 ...
 * @return  float   运费
 */
function shipping_fee($suppid,$shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='')
{
    ...
    if (file_exists($filename))
    {
        ...
        if($suppid != 0) {
            $region = $GLOBALS['db']->getRow("select s.origin_code from ". $GLOBALS['ecs']->table('supplier') ." s where s.supplier_id='".$suppid."' " );
        }
        
        // echo "<pre>";var_dump($region); die();

        # Get address ID yangd id pakai berdasarkan userID di table ecs _user
        # Join ke ecs Region untuk mendapatkan dest_code

        $address = $GLOBALS['db']->getRow("select address_id from ". $GLOBALS['ecs']->table('users') ." where user_id='".$_SESSION['user_id']."' " );
        $user_address = $GLOBALS['db']->getRow("select district from ". $GLOBALS['ecs']->table('user_address') ." where address_id='".$address['address_id']."' " );
        $row = $GLOBALS['db']->getRow("select dest_code from ". $GLOBALS['ecs']->table('region') ." where region_id='".$user_address['district']."' " );
        
        $destination = $row['dest_code'];
        $from = $region['origin_code'];

        if($suppid == 0) {
            $from = 'CGK10000';
        }

        if($shipping_code === 'jne') {
            return $obj->calculate($from, $goods_weight, $goods_amount, $goods_number, $destination);
...
function order_fee($order, $goods, $consignee)
{
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
        ...
        foreach ($goods AS $val)
        {
           ...
           $curl_shipping_fee = shipping_fee($row_supp['supplier_id'], $shipping_info['shipping_code'],$shipping_info['configure'], $weight_price2['weight'], $total['goods_price_supplier'][$row_supp['supplier_id']], $weight_price2['number']);
           $shipp = explode("#", $curl_shipping_fee);

           $total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee'] = ($shipping_count_supp == 0 AND $weight_price2['free_shipping'] == 1) ?0 : (int)$shipp[0];
        ...
        // var_dump($total['supplier_shipping']);  
    ...
    else
    {
        ...
        if(isset($total['bonus_kill']))
        {
            ...
            $total['amount'] -=  $price = ($total['bonus_kill'] > 0 ? number_format($total['bonus_kill'], 0, ',', '.') : 0); // 还需要支付的订单金额
        }
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			...
			" IF(ga.act_id, IFNULL(ss.supplier_name, '".$GLOBALS['_LANG']['m_409']."'), IFNULL(s.supplier_name, '".$GLOBALS['_LANG']['m_409']."')) as seller " .
...
function get_cart_goods($other='')
{
	...
	if($row['supplier_id'])
	{
		...
		$supplier_name = $GLOBALS['_LANG']['suppliers_name'].' '. $supplier_name;
	}
	else
	{
		$supplier_name = $GLOBALS['_LANG']['m_409'];
	}
