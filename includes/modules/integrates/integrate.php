...
class integrate
{
	 ...
	 * @return int
	 */
	function add_user_by_mobile ($username, $password, $mobile,$pre_mobile_phone='', $gender = -1, $bday = 0, $reg_date = 0, $md5password = '')
	{
		/* 将用户添加到整合方 */
		if($this->check_user($username) > 0)
		{
		...
		/* 补充手机已验证 */
		$fields = array(
			$this->field_name,$this->field_mobile_phone,$this->field_pre_mobile_phone,$this->field_pass,$this->field_mobile_validated
		);
		$values = array(
			$post_username,$mobile,$pre_mobile_phone,$post_password,1
		);
