...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && ($order['mobile'] != '' || $order['tel'] != ''))
        {
			...
			$content = $_LANG['x_196'].$order['order_sn'].$_LANG['x_197'].local_date("Y-m-d H:i:s",time()).$_LANG['x_198'];
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    $order = array();
        ...
        else
        {
            ...
            {
                $msg_detail = $_LANG['x_190'];
                $msg_type = '0';
            }
            else
            {
                $msg_detail = $_LANG['x_191'];
                $msg_type = '1';
            }
            $link_text = $_LANG['x_200'];
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('invoice' == $step)
    {
        ...
        if(!empty($_REQUEST['step_detail']))
        {
            ...
            {
                $action_link = array('text'=>$_LANG['x_201'],'href'=>'order.php?act=invoice_list&uselastfilter=1');
...
elseif ($_REQUEST['act'] == 'operate')
{
   ...
   $msg = '';
    ...
    elseif (isset($_POST['ship']))
    {
        ...
        {
            $order['shipping_name'] = $_LANG['x_203'];
        }
...
elseif($_REQUEST['act'] == 'provide_invoice')
{
    ...
    else
    {
        ...
        {
            $link_text = $_LANG['x_201'];
            $link_href = 'order.php?act=invoice_list';
            $msg_detail = $_LANG['x_204'];
            $msg_type = '0';
            $links = array(array('text'=>$link_text,'href'=>$link_href));
            $auto_redirect = true;
            sys_msg($msg_detail, $msg_type,$links, $auto_redirect);
...
function format_invoice_rows($rows)
{
    ...
    {
        $_LANG['provided'] = $_LANG['x_207'];
        $_LANG['unprovided'] = $_LANG['x_208'];
        $_LANG['normal_invoice'] = $_LANG['x_209'];
        $_LANG['vat_invoice'] = $_LANG['x_210'];
        $temp = array();
...
function export_invoice_to_excel($rows)
{
    ...
    if(is_array($rows) && count($rows) > 0)
    {
        $filename = $_LANG['x_201'].date('Ymd').'.xls';
        $filename =  iconv(mb_detect_encoding($filename, mb_detect_order(), true), "gbk", $filename);
        header("Content-Disposition: attachment; filename=$filename");
        header("Content-type:application/vnd.ms-excel; charset=gbk");
        $data .= '<table border="1">';
        $data .= '<tr><th>'.$_LANG['x_202'].'</th><th>'.$_LANG['x_212'].'</th><th>'.$_LANG['x_213'].'</th>'.
                '<th>'.$_LANG['x_214'].'</th><th>'.$_LANG['x_215'].'</th><th>'.$_LANG['x_216'].'</th>'.
                '<th>'.$_LANG['x_217'].'</th><th>'.$_LANG['x_218'].'</th><th>'.$_LANG['x_219'].'</th>'.
                '<th>'.$_LANG['x_220'].'</th><th>'.$_LANG['x_221'].'</td><th>'.$_LANG['x_222'].'</th>'.
                '<th>'.$_LANG['x_223'].'</th><th>'.$_LANG['x_224'].'</th><th>'.$_LANG['x_225'].'</th><th>'.$_LANG['x_226'].'</th></tr>';
...
function export_invoice_to_xml_excel($rows)
{
    ...
    if(is_array($rows) && count($rows) > 0)
    {
        $arr = array('order_sn' => array('name' => $_LANG['x_211'],'type' => 'String','width' => '100'),
            'add_time' => array('name' => $_LANG['x_212'],'type' => 'String','width' => '100'),
            'inv_status' => array('name' =>$_LANG['x_213'],'type' => 'String','width' => '50'),
            'inv_payee' => array('name' => $_LANG['x_215'],'type' => 'String','width' => '100'),
            'inv_content' => array('name' => $_LANG['x_216'],'type' => 'String','width' => '60'),
            'inv_money' => array('name' => $_LANG['x_226'],'type' => 'String','width' => '80'),
            'vat_inv_company_name' => array('name' =>$_LANG['x_217'],'type' => 'String','width' => '120'),
            'vat_inv_taxpayer_id' => array('name' => $_LANG['x_218'],'type' => 'String','width' => '100'),
            'vat_inv_registration_address' => array('name' =>$_LANG['x_219'],'type' => 'String','width' => '200'),
            'vat_inv_registration_phone' => array('name' => $_LANG['x_220'],'type' => 'String','width' => '80'),
            'vat_inv_deposit_bank' => array('name' => $_LANG['x_221'],'type' => 'String','width' => '100'),
            'vat_inv_bank_account' => array('name' =>$_LANG['x_222'],'type' => 'String','width' => '150'),
            'inv_consignee_name' => array('name' => $_LANG['x_223'],'type' => 'String','width' => '100'),
            'inv_consignee_phone' => array('name' => $_LANG['x_224'],'type' => 'String','width' => '80'),
            'inv_consignee_address' => array('name' => $_LANG['x_225'],'type' => 'String','width' => '200'));
        ...
        $normal_inv_sheet = '<ss:Worksheet ss:Name="'.$_LANG['x_209'].'"><ss:Table>';
        ...
        $vat_inv_sheet = '<ss:Worksheet ss:Name="'.$_LANG['x_210'].'"><ss:Table>';
        ...
        foreach($rows as $row)
        {
            ...
            elseif($row['inv_type'] == $_LANG['x_210'])
            {
                $has_vat_inv = true;
...
function new_kuaidi_order($delivery_id)
{
	...
	/* 快递单状态 */
	$orderstatus_array = array( '1'=> array('name'=>$_LANG['x_227'], 'type'=>'0'),
								'2'=> array('name'=>$_LANG['x_228'], 'type'=>'0'),
								'3'=> array('name'=>$_LANG['x_229'], 'type'=>'0'),
								'4'=> array('name'=>$_LANG['x_230'], 'type'=>'1'),
								'5'=> array('name'=>$_LANG['x_231'], 'type'=>'2'),
								'6'=> array('name'=>$_LANG['x_232'], 'type'=>'2'),
								'7'=> array('name'=>$_LANG['x_233'], 'type'=>'3'), );
...
function get_distrib_info_by_id($order_id)
{
	 ...
	 for ($i=0; $i < $num; $i++)
	 {
		 ...
		 $arr[$i]['level'] = ($i+1).$GLOBALS['_LANG']['x_234'];
