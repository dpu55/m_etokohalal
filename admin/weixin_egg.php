...
switch ($act){
	case "list"://list
		...
		if($_POST){
			...
			$link [] = array ('href' => 'weixin_egg.php?act=list','text' => $_LANG['x_11']);
			sys_msg ( $_LANG['x_190'], 0, $link );
		}elseif($aid > 0){
			$act = $db->getRow ( "SELECT * FROM  " . $ecs->table('weixin_act') . "  where aid=$aid" );
			$smarty->assign('action_link',  array('text' => $_LANG['x_237'], 'href'=>'weixin_egg.php?act=listall&aid='.$aid));
	...
	case "add"://add and edit
		...
		if($_POST){
			$ret = $db->query($sql);
			$link [] = array ('href' => 'weixin_egg.php?act=list&aid='.$aid,'text' =>$_LANG['x_11']);
			sys_msg ( $_LANG['x_190'], 0, $link );
		}else{
	...
	case "log":
		...
		if($lid > 0 && $tag == 'send'){
			...
			$link [] = array ('href' => 'weixin_egg.php?act=log','text' => $_LANG['x_239']);
			sys_msg ( $_LANG['x_190'], 0, $link );
		}
