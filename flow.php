 ...
 * 鸿宇多用户商城 购物流程
 * ============================================================================
 * 版权所有 2015-2018 HongYu科技有限公司，并保留所有权利。
...
elseif ($_REQUEST['step'] == 'checkout')
{
		...
		if($favourable_list){
			...
			foreach($favourable_list as $fk=>$fv){
				if(!$fv['available']){
					...
					if($db->getOne($sql) > 0){
						show_message($_LANG['m_616'].' ['.$fv['act_name'].']'.$_LANG['m_617'], '', '', 'warning');
					}
				}
			}
			unset($sql_where);
            ...
            foreach($goods_list as $k => $v)
            {
                if($v['is_buy'] == 1 && $v['buymax'] >0 && $v['buymax_start_date'] < $time_xg_now  && $v['buymax_end_date'] > $time_xg_now )
                {
                    ...
                    if ( $num_total > intval($v['buymax']) )
                    {
                        ...
                        $message .= $_LANG['m_365']." <font color=#330099>【".$v['goods_name']."】</font> ".$_LANG['m_618']." <font color=#330099>". $v['buymax'] . "</font> 件<br>";
                        if ($num_cart_old)
                        {
                            $message .= $_LANG['m_619']." <font color=#330099>$num_cart_old</font> <br>";
                        }
                        $message .= $_LANG['m_620']." <font color=#330099>". $num_else ."</font> <br>";
                    }
                }
            }
            if($message != '')
	...
	$consignee['region'] = $address['district'].', '.$address['city'].', '.$address['province'].', '.country($address['country']);
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	foreach($res as $key=>$value)
	{
		...
		{
			show_message($_LANG['m_181']."  ".$value['goods_name']."  ".$_LANG['m_621']);
		}
	}
...
if($_REQUEST['step']=='update_group_cart')
{
	...
	{
	$result['error'] = '1';
	$result['content'] =$_LANG['m_625'].$goods_number."";
