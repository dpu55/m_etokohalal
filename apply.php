...
if(isset($_POST['do']) && $_POST['do']){
	...
	if($shownum == 1){
		...
		if(count($save1)!=count($save)){
			show_message($_LANG['m_f_1045'], $_LANG['m_f_1046'], 'apply.php', 'wrong');
			}
			 if ($db->autoExecute($ecs->table('supplier'), $save, 'UPDATE', 'user_id='.$userid) !== false){
				header("location:apply.php");
				exit;
			 }else{
				show_message($_LANG['m_f_1047'], $_LANG['m_f_1046'], 'apply.php', 'wrong');
			 }
		}
		if($_POST['person'])
		{
		...
		if(isset($_POST['input_apply_agreement']) && intval($_POST['input_apply_agreement']) > 0){
			...
			if($info){
				if ($db->autoExecute($ecs->table('supplier'), $apply, 'UPDATE', 'user_id='.$userid) !== false){
				 	...
				 	show_message($_LANG['m_f_1053'], $_LANG['m_f_1046'], 'apply.php', 'wrong');
				 }
			}else{
...
$smarty->assign('var_path',$img_url);
