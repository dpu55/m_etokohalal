...
function action_check_username ()
{
	...
	if(empty($username))
	{
		show_message($_LANG['m_510'], $_LANG['m_166'], 'findPwd.php?act=index', 'info');
	}
	...
	if(is_mobile_phone($username))
	{
		...
		if($index > 1)
		{
			show_message($_LANG['m_511'], $_LANG['relogin_lnk'], 'findPwd.php', 'error');
		}
		else if($index == 1)
		{
			if($user_id)
	...
	if(! $username_exist)
	{
		show_message($_LANG['m_512'], $_LANG['relogin_lnk'], 'findPwd.php', 'error');
	}
	
	// 获取用户信息，判断用户是否验证了手机、邮箱
	$sql = "select user_id, user_name, email, mobile_phone,pre_mobile_phone from " . $ecs->table('users') . " where user_id = '" . $user_id . "'";
	...
	if(isset($row['mobile_phone']) && ! empty($row['mobile_phone']))
	{
        ...
        $pre_mobile_phone = $row['pre_mobile_phone'];
		$validate_types[] = array(
			'type' => 'mobile_phone','name' => $_LANG['m_513'],'value' => $pre_mobile_phone.$mobile_phone
		);
	}
	if(isset($row['email']) && ! empty($row['email']))
	{
		...
		$validate_types[] = array(
			'type' => 'email','name' => $_LANG['m_156'],'value' => $email
		);
	}
...
function action_validate ()
{
	...
	if(!isset($_SESSION['find_password']))
	{
		...
		exit(json_encode(array('error' => 1, 'content' => $_LANG['m_514'], 'url' => 'findPwd.php')));
	}
	...
	{
		exit(json_encode(array('error' => 1, 'content' => $_LANG['m_515'], 'url' => 'findPwd.php')));
	}
	...
	{
		/* 无效的注册类型 */
		exit(json_encode(array('error' => 1, 'content' => $_LANG['m_516'], 'url' => 'findPwd.php')));
	}
...
function action_reset_password()
{
	...
	{
		exit(json_encode(array('error' => 1, 'content' => $_LANG['m_154'], 'url' => '')));
	}
	...
	if($result == false)
	{
		exit(json_encode(array('error' => 1, 'content' => $_LANG['m_517'], 'url' => '')));
	}
	else 
	{
